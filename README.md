...
languages via tree-sitter), and the hotspot table's CHN column shows each
file's HEAD-anchored commit count via
[richardwooding/gitmeta](https://github.com/richardwooding/gitmeta) — high
churn on a high-complexity function is the classic refactor-priority signal. Vendored, minified, and generated files are
excluded via [richardwooding/projectdetect] — generated code (protobuf
output, mocks, `.gen.go` files, and anything opening with a
`Code generated` / `@generated` / `<auto-generated>` header) is skipped by
default so tool output doesn't drown the metrics; pass `--include-generated`
to keep it.
...
gitlapse --include-generated # count protobuf/mock/codegen output too
```
